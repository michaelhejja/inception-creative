...
if($bg_select == 'bg_video' && (!empty($ogv_url) || !empty($mp4_url))){
    ...
    //$video_html .= '<div class="'.join(" ",$video_classes).'">';
    //$video_html .= '<video id="'.uniqid().'" class="video-background"'.$video_mute.' preload="auto" width="1700" height="970" preload="auto" autoplay loop playsinline>';
    ...
    $video_lazy_offset = (isset($video_lazy_offset) && is_numeric($video_lazy_offset)) ? $video_lazy_offset : 500;
    $video_lazy_play = (isset($video_lazy_play) && is_numeric($video_lazy_play)) ? $video_lazy_play : 55;
    $video_mute = (isset($video_mute) && $video_mute == 1) ? "true" : "false";
    $data_array = array(
