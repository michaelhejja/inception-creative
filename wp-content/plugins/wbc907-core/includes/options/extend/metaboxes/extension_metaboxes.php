        ...
        class ReduxFramework_extension_metaboxes {
            ...
            public function __construct( $parent ) {
                ...
                if ( empty( self::$_extension_dir ) ) {
                    ...
                    if( ABSPATH && ABSPATH == '//' ){
                        $this->_extension_url = site_url( trailingslashit( $this->_extension_dir ) );
                    }else{
            ...
            public function init() {
                ...
                //Adds to better select the post id, then the default redux metabox function which 
                //seems very unreliable. Great as this will add further features/options :)
                $this->post_id = intval( apply_filters( 'redux_post_meta_id', $this->post_id ) );
...
//                        );
            ...
            function url_to_postid( $url ) {
                ...
                if ( preg_match( '#[?&](p|post_id|post|page_id|attachment_id)=(\d+)#', $url, $values ) ) {
                    $id = absint( $values[2] );
                    if ( $id ) {
                        return $id;
                    }
                ...
                // Added - Checks to see if is on front page with 'page' set as 
                // homepage. 
                // 
                // If you add a variable to the url IE: yousite.com/?some_var=var
                // it will cause the page to not load meta options/settings
                // 
                $public_query_vars = array('m', 'p', 'posts', 'w', 'cat', 'withcomments', 'withoutcomments', 's', 'search', 'exact', 'sentence', 'debug', 'calendar', 'more', 'tb', 'pb', 'author', 'order', 'orderby', 'year', 'monthnum', 'day', 'hour', 'minute', 'second', 'name', 'category_name', 'tag', 'feed', 'author_name', 'static', 'pagename', 'page_id', 'error', 'comments_popup', 'attachment', 'attachment_id', 'subpost', 'subpost_id', 'preview', 'robots', 'taxonomy', 'term', 'cpage', 'post_type'); 
                
                if ( isset( $_GET ) && is_array( $_GET ) ) {
                    foreach ( $_GET as $key => $value ) {
                        if( in_array( $key, $public_query_vars ) ) 
                            return 0;
                    }
                }
                
                $url_query = explode( '?', $orig_url );
                $url       = trailingslashit( $url_query[0] );

                if( 'page' == get_option( 'show_on_front' ) && $url == get_permalink( get_option( 'page_on_front' ) ) ){
                    return get_option( 'page_on_front' );
                }
                                ...
                                <ul class="redux-group-menu">
                                    <?php
                                        foreach ( $sections as $sKey => $section ) {
                                            $sectionMenu = $this->parent->section_menu( $sKey, $section, '_box_' . $metabox['id'], $sections );
                                            
                                            echo !empty( $sectionMenu ) ? $sectionMenu : '';
                                        }
                                    ?>
                                </ul>
                            </div>
